...
export class InputManager {
  ...
  private handleTouchStart(event: TouchEvent): void {
    ...
    if (!this.gameState.isFullscreen) {
      ...
      if (event.touches.length === 2) {
        ...
        this.touchState.lastTouchRotation = Math.atan2(touch2.clientY - touch1.clientY, touch2.clientX - touch1.clientX)
      }
    }
  }
  ...
  private handleTouchMove(event: TouchEvent): void {
    ...
    if (event.touches.length === 1) {
      ...
      // 회전 각도 계산
      const currentRotation = Math.atan2(touch2.clientY - touch1.clientY, touch2.clientX - touch1.clientX)
